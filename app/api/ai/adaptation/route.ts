...
import { analyzeWellnessAndAdapt, generateMotivationalMessage, generateFormCues } from '@/lib/utils/ai/adaptation'
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (recentSetLogs.length === 0) {
      ...
      const { data: setLogs } = await supabase
        ...
        .in('session_exercise_id', 
          recentSessions.flatMap(s => s.session_exercises?.map(se => se.id) || [])
        )
    ...
    const motivationalMessage = await generateMotivationalMessage({
      ...
      current_challenges: await getCurrentChallenges(wellnessData, recentSessions)
    })
...
export async function POST_MOTIVATIONAL(request: NextRequest) {
  try {
    ...
    const motivationalMessage = await generateMotivationalMessage(performanceData)
 ...
 */
async function getRecentImprovements(userId: string, supabase: any): Promise<string[]> {
  try {
    ...
    if (latest.value > previous.value) {
      improvements.push(`Improved ${latest.metric_type} by ${((latest.value - previous.value) / previous.value * 100).toFixed(1)}%`)
    }
 ...
 * Get upcoming goals
 */
async function getUpcomingGoals(userId: string, supabase: any): Promise<string[]> {
  try {
 ...
 */
async function getCurrentChallenges(wellnessData: any, recentSessions: any[]): Promise<string[]> {
  ...
  if (recentSessions.length > 0) {
    const avgRPE = recentSessions.reduce((sum, session) => 
      sum + (session.average_rpe || 0), 0) / recentSessions.length
