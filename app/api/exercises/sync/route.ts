  ...
  try {
    ...
    if (source === 'both' || source === 'exercisedb') {
      try {
        ...
        const exercisedbResult =
          await exerciseDBService.syncExercisesToDatabase(supabase, batch_size);
        results.exercisedb = exercisedbResult;
        console.log(
          `✅ ExerciseDB sync complete: ${exercisedbResult.synced} synced, ${exercisedbResult.errors} errors`
        );
      } catch (error) {
    ...
    if (source === 'both' || source === 'youtube') {
      try {
        ...
        console.log(
          `✅ YouTube sync complete: ${youtubeResult.synced} synced, ${youtubeResult.errors} errors`
        );
      } catch (error) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = {
      ...
      video_coverage: totalExercises
        ? Math.round(((exercisesWithVideos || 0) / totalExercises) * 100)
        : 0,
    };
    ...
    for (const exercise of exercises) {
      try {
        ...
        if (videos.length > 0) {
          ...
          if (updateError) {
            console.error(
              `Error updating exercise ${exercise.id}:`,
              updateError
            );
            errors++;
          } else {
