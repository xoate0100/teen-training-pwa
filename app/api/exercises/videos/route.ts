...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!exerciseName && !category && !muscleGroup && !equipment && !difficulty) {
    ...
    if (exerciseName) {
      videos = await youtubeService.searchExerciseDemonstration(exerciseName, maxResults)
    }
    ...
    // Search based on muscle group
      ...
      videos = await youtubeService.getMuscleGroupVideos(muscleGroup, maxResults)
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { query, max_results = 10, order = 'relevance', duration = 'short' } = body
