...
import { analyzeSafetyMetrics, generateSafetyAlerts, shouldModifySession } from '@/lib/utils/safety-monitoring'
  ...
  try {
    ...
    const summaryMetrics = {
      recent_energy: recentCheckIns?.length ? 
        recentCheckIns.reduce((sum, ci) => sum + ci.energy_level, 0) / recentCheckIns.length : 0,
      recent_soreness: recentCheckIns?.length ? 
        recentCheckIns.reduce((sum, ci) => sum + ci.muscle_soreness, 0) / recentCheckIns.length : 0,
      recent_sleep: recentCheckIns?.length ? 
        recentCheckIns.reduce((sum, ci) => sum + ci.sleep_hours, 0) / recentCheckIns.length : 0,
      recent_rpe: recentSessions?.length ? 
        recentSessions.reduce((sum, s) => sum + (s.average_rpe || 0), 0) / recentSessions.length : 0,
