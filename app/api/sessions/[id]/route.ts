...
import { UpdateSessionRequest, Session, ApiResponse } from '@/lib/types/database'
...
export async function PUT(
  ...
  try {
    ...
    if (status === 'completed') {
      ...
      if (sessionExercises) {
        const totalSets = sessionExercises.reduce((sum, se) => sum + se.sets, 0)
        const totalReps = sessionExercises.reduce((sum, se) => sum + (se.sets * se.reps), 0)
        
        // Calculate average RPE from set logs
        const allSetLogs = sessionExercises.flatMap(se => se.set_logs || [])
        const averageRpe = allSetLogs.length > 0 
          ? allSetLogs.reduce((sum, log) => sum + log.rpe, 0) / allSetLogs.length
          : null
