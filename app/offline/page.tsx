    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        ...
        <CardContent className="space-y-6">
          ...
          {/* Pending Data */}
          {(pendingData.sessions > 0 || pendingData.checkins > 0 || pendingData.progress > 0) && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <h3 className="font-medium text-blue-900 mb-2">Pending Data</h3>
              <div className="space-y-1 text-sm text-blue-700">
                {pendingData.sessions > 0 && (
                  <div>• {pendingData.sessions} workout session{pendingData.sessions !== 1 ? 's' : ''}</div>
                )}
                {pendingData.checkins > 0 && (
                  <div>• {pendingData.checkins} daily check-in{pendingData.checkins !== 1 ? 's' : ''}</div>
                )}
                {pendingData.progress > 0 && (
                  <div>• {pendingData.progress} progress update{pendingData.progress !== 1 ? 's' : ''}</div>
                )}
              </div>
          ...
          <div className="text-center">
            <p className="text-xs text-gray-500">
              Don't worry! Your progress is being saved locally and will sync when you're back online.
            </p>
          </div>
        </CardContent>
      </Card>
