...
import { Play, Pause, RotateCcw, Trophy, Star, Zap, Target } from "lucide-react"
...
export function MicroBreakTimer({ onComplete }: { onComplete: () => void }) {
  ...
  const brainBreakGames = [
    { name: "Deep Breathing", instruction: "Take 5 deep breaths in and out", icon: "🫁" },
    { name: "Shoulder Rolls", instruction: "Roll your shoulders 10 times", icon: "💪" },
    { name: "Eye Circles", instruction: "Look up, down, left, right slowly", icon: "👀" },
    { name: "Gentle Stretch", instruction: "Reach up high and stretch your sides", icon: "🤸" },
  ]
    ...
    <Card className="bg-gradient-to-br from-blue-50 to-purple-50 border-2 border-blue-200">
      ...
      <CardContent className="space-y-4">
        <div className="text-center">
          <div className="text-3xl font-bold text-purple-600 mb-2">{timeLeft}s</div>
...
export function ChoiceStation({ onSelect }: { onSelect: (choice: string) => void }) {
  const drillOptions = [
    {
      name: "Agility Ladder",
...
export function RandomCueDisplay() {
  ...
  const motivationalCues = [
    ...
    { text: "Stay balanced and controlled!", icon: "⚖️", color: "text-purple-600" },
  ]
    ...
    <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50 animate-bounce">
      <Card className="bg-gradient-to-r from-pink-100 to-rose-100 border-2 border-pink-300 shadow-lg">
        <CardContent className="p-3 text-center">
          <div className={`text-lg font-bold ${cue.color} flex items-center gap-2`}>
    ...
    <Card className="bg-gradient-to-br from-purple-100 to-pink-100 border-2 border-purple-300">
      ...
      <CardContent className="space-y-4">
        <div className="text-center">
          ...
          <h3 className="font-bold text-lg text-purple-800">{weeklyChallenge.title}</h3>
        ...
        <div className="space-y-2">
          ...
          <Progress value={(weeklyChallenge.daysCompleted / weeklyChallenge.totalDays) * 100} className="h-3" />
        </div>

        <div className="bg-white p-3 rounded-lg border border-purple-200">
          <div className="flex items-center gap-2 text-sm">
            <Star className="w-4 h-4 text-yellow-500" />
            <span className="font-medium">Reward: {weeklyChallenge.reward}</span>
          </div>
        </div>
    ...
    <Card className="bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-300">
      ...
      <CardContent className="space-y-4">
          ...
          {leaderboardData.map((user, index) => (
            <div
              ...
              className={`flex items-center justify-between p-3 rounded-lg border-2 ${
                user.isUser ? "bg-yellow-100 border-yellow-300 border-dashed" : "bg-white border-gray-200"
              }`}
            >
              <div className="flex items-center gap-3">
                <div className="text-2xl">{user.avatar}</div>
                <div>
                  <div className={`font-semibold ${user.isUser ? "text-yellow-700" : "text-gray-700"}`}>
                    #{index + 1} {user.name}
                  </div>
        ...
        <div className="bg-yellow-100 p-3 rounded-lg border-2 border-yellow-300">
          <h4 className="font-semibold text-yellow-700 mb-2">🎉 Encouragement Corner</h4>
