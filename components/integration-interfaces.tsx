    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <Button onClick={handlePhotoCapture} variant="outline" className="flex items-center gap-2 bg-transparent">
            <Camera className="h-4 w-4" />
            Take Photo
          </Button>
          <Button onClick={handleVideoCapture} variant="outline" className="flex items-center gap-2 bg-transparent">
...
export function SocialSharing() {
  const [achievements] = useState([
    { id: 1, title: "First Week Complete!", description: "Completed 6 training sessions", icon: "🎯" },
    { id: 2, title: "Strength Gains", description: "Increased squat weight by 10lbs", icon: "💪" },
    { id: 3, title: "Perfect Attendance", description: "No missed sessions this week", icon: "⭐" },
  ])
...
export function CalendarIntegration() {
  const [upcomingSessions] = useState([
    { date: "2024-01-15", time: "07:00", type: "Lower Body Strength", duration: "45 min" },
    { date: "2024-01-15", time: "16:00", type: "Volleyball Skills", duration: "60 min" },
    { date: "2024-01-16", time: "07:00", type: "Upper Body Strength", duration: "45 min" },
  ])
      ...
      <CardContent className="space-y-4">
        {upcomingSessions.map((session, index) => (
          <div key={index} className="flex items-center justify-between p-3 border rounded-lg">
            ...
            <Button size="sm" variant="outline" onClick={() => addToCalendar(session)}>
...
export function DataExport() {
  const [exportOptions] = useState([
    { type: "progress", label: "Progress Report", description: "Weekly progress summary for coaches/parents" },
    { type: "sessions", label: "Session Data", description: "Complete workout logs and performance metrics" },
    { type: "wellness", label: "Wellness Tracking", description: "Daily check-ins and recovery data" },
    { type: "achievements", label: "Achievements", description: "Badges, milestones, and accomplishments" },
  ])

  const exportData = (type: string) => {
    const mockData = {
      progress: { weeks: 4, sessions: 24, avgRPE: 6.8, improvements: ["Squat +15lbs", "Jump +2in"] },
      sessions: { totalSessions: 24, avgDuration: "47min", topExercises: ["Goblet Squat", "Push-ups"] },
      wellness: { avgMood: 4.2, avgEnergy: 7.8, avgSleep: 8.1, streakDays: 12 },
      achievements: { badges: 8, milestones: 3, weeklyGoals: 4 },
    }

    const dataBlob = new Blob([JSON.stringify(mockData[type as keyof typeof mockData], null, 2)], {
      ...
      <CardContent className="space-y-4">
        {exportOptions.map((option) => (
          <div key={option.type} className="flex items-center justify-between p-3 border rounded-lg">
            ...
            <Button size="sm" variant="outline" onClick={() => exportData(option.type)}>
