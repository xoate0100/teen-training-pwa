...
export function IntelligentInput({
  ...
  const handleVoiceInput = () => {
    ...
    recognition.onresult = (event: any) => {
      ...
      if (
        !transcript.includes('rpe') &&
        !transcript.includes('weight') &&
      ...
      <CardHeader>
        ...
        {previousData && (
          <div className='text-sm text-muted-foreground'>
            Last time: {previousData.weight}lbs × {previousData.reps} reps (RPE{' '}
            {previousData.rpe})
          </div>
        )}
      </CardHeader>
      <CardContent className='space-y-4'>
        ...
        <div>
          <div className='flex justify-between items-center mb-2'>
            ...
            <span className={`text-lg font-bold ${getRpeColor(rpe)}`}>
              {rpe}/10
            </span>
          </div>
          <Slider
            ...
            className='mb-2'
          />
          <div className={`text-center text-sm ${getRpeColor(rpe)}`}>
          ...
          <Button
            ...
            onClick={() => {
              const utterance = new SpeechSynthesisUtterance(
                `Set ${currentSet} complete. RPE ${rpe}`
              );
