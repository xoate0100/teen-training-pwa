...
export function PredictiveUserExperienceDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'interface' | 'content' | 'recommendations' | 'paths'>('interface');
  ...
  }, [autoAnalysis, selectedUserId, generateAnticipatoryChanges, generateProactiveContent, generateSmartRecommendations, generateLearningPaths]);
  ...
  const getContentTypeIcon = (contentType: string) => {
    switch (contentType) {
      case 'workout': return <Activity className="h-5 w-5" />;
  ...
  const getRecommendationTypeIcon = (recommendationType: string) => {
    switch (recommendationType) {
      ...
      case 'nutrition': return <Droplets className="h-5 w-5" />;
  ...
  const getPathTypeIcon = (pathType: string) => {
    switch (pathType) {
      case 'strength': return <Zap className="h-5 w-5" />;
      case 'volleyball': return <Users className="h-5 w-5" />;
      case 'plyometric': return <TrendingUp className="h-5 w-5" />;
      case 'recovery': return <Heart className="h-5 w-5" />;
  ...
  const getUrgencyColor = (urgency: string) => {
    switch (urgency) {
      ...
      case 'later': return 'text-yellow-600';
                ...
                <div key={index} className="p-4 border rounded-lg">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="font-medium">{change.changeType.charAt(0).toUpperCase() + change.changeType.slice(1)} Change</h4>
                    </div>
                    <div className="flex gap-2">
                  ...
                  <p className="text-sm text-muted-foreground mb-3">
                    {change.change.component} - {change.change.property}: {JSON.stringify(change.change.value)}
                  </p>
                  ...
                  {change.change.animation && (
                    <div className="text-xs text-muted-foreground">
                      Animation: {change.change.animation} ({change.change.duration}ms)
                    </div>
                  )}
                </div>
              ))}
            ...
            <div className="space-y-4">
              {proactiveContent.map((content, index) => (
                <div key={index} className="p-4 border rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground mb-3">{content.content.description}</p>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs text-muted-foreground">
                  ...
                  {content.content.media && (
                    <div className="text-xs text-muted-foreground mb-2">
                      Media: {content.content.media.type} - {content.content.media.url}
                    </div>
                  )}
                  {content.content.action && (
                    <div className="text-xs text-muted-foreground">
                      Action: {content.content.action.type} → {content.content.action.target}
                    </div>
                  )}
                </div>
              ))}
                ...
                <div key={index} className="p-4 border rounded-lg">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="font-medium">{recommendation.recommendation.title}</h4>
                    </div>
                    <div className="flex gap-2">
                      <Badge className={getPriorityColor(recommendation.priority)}>
                        {recommendation.priority}
                      </Badge>
                      <Badge className={getUrgencyColor(recommendation.timing.urgency)}>
                        {recommendation.timing.urgency}
                      </Badge>
                    </div>
                  </div>
                  <p className="text-sm text-muted-foreground mb-3">{recommendation.recommendation.description}</p>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs text-muted-foreground">
                      Confidence: {Math.round(recommendation.trigger.confidence * 100)}%
                    </span>
                    <span className="text-xs text-muted-foreground">
                      Optimal Time: {recommendation.timing.optimalTime.toLocaleString()}
                    </span>
                  </div>
                  <div className="text-xs text-muted-foreground mb-2">
                  ...
                  {recommendation.recommendation.alternatives.length > 0 && (
                    <div className="text-xs text-muted-foreground">
                      Alternatives: {recommendation.recommendation.alternatives.map(alt => 
                        typeof alt === 'string' ? alt : alt.type
                      ).join(', ')}
                    </div>
                  )}
                </div>
                ...
                <div key={index} className="p-4 border rounded-lg">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Badge variant="outline">
                        {path.path.difficulty}/10
                      </Badge>
                    </div>
                  </div>
                  <p className="text-sm text-muted-foreground mb-3">{path.path.description}</p>
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs text-muted-foreground">
                  ...
                  <div className="space-y-2">
                    ...
                    {path.path.phases.map((phase, phaseIndex) => (
                      <div key={phaseIndex} className="text-xs text-muted-foreground pl-4">
                        <div className="font-medium">{phase.name} ({phase.duration} weeks)</div>
                      ...
                      <div className="text-xs text-muted-foreground space-y-1">
                        {path.path.adaptations.map((adaptation, adaptIndex) => (
                          <div key={adaptIndex} className="pl-4">
                            {adaptation.type}: {adaptation.condition} → {adaptation.adjustment}
                          </div>
                        ))}
                      </div>
                    </div>
