...
import { Trophy, TrendingUp, Target, Zap, Star, Medal, Crown } from "lucide-react"
...
export function ProgressChart() {
  ...
  useEffect(() => {
    const loadProgressData = () => {
      ...
      const calculatedAchievements = calculateAchievements(sessions, checkIns, currentWeekStats)
  ...
  const calculateCurrentWeekStats = (sessions: SessionData[], checkIns: any[]) => {
    ...
    const averageRPE =
      thisWeek.length > 0 ? thisWeek.reduce((sum, session) => sum + session.totalRPE, 0) / thisWeek.length : 0
      ...
      (sum, session) =>
        ...
        session.exercises.reduce(
          (exerciseSum, exercise) =>
            exerciseSum + exercise.sets.reduce((setSum, set) => setSum + set.reps * (set.weight || 1), 0),
          0,
        ),
      0,
    )
  ...
  const calculateAchievements = (sessions: SessionData[], checkIns: any[], stats: any) => {
    const baseAchievements = [
      ...
      },
      {
        ...
        maxProgress: 3,
        unlockedAt: stats.sessionsCompleted >= 3 ? new Date().toISOString() : undefined,
      },
      {
        id: "consistency-king",
      ...
      },
      {
        ...
        maxProgress: 500,
        unlockedAt: stats.totalVolume >= 500 ? new Date().toISOString() : undefined,
      },
      {
        id: "intensity-expert",
                ...
                <div className="flex items-start gap-4">
                  <div
                    className={`p-3 rounded-full ${
                      achievement.unlocked ? "bg-yellow-400 text-yellow-900" : "bg-gray-200 text-gray-500"
                    }`}
                  >
                  ...
                  <div className="flex-1">
                    <h3 className={`font-bold text-lg ${achievement.unlocked ? "text-yellow-900" : "text-gray-600"}`}>
                      {achievement.title}
                    </h3>
                    <p className={`text-sm mb-3 ${achievement.unlocked ? "text-yellow-800" : "text-gray-500"}`}>
          ...
          <CardContent className="p-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-3xl font-bold text-primary">{weeklyStats.sessionsCompleted}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-2 border-orange-200 hover:border-orange-300 transition-colors">
          <CardContent className="p-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-sm font-medium text-muted-foreground">Avg RPE</p>
                <p className="text-3xl font-bold text-orange-600">{weeklyStats.averageRPE}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-3xl font-bold text-green-600">{weeklyStats.totalVolume}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-3xl font-bold text-yellow-600">{weeklyStats.streak}</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="space-y-3">
              {recentSessions.map((session, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                  ...
                  <Badge
                    variant={session.totalRPE <= 6 ? "secondary" : session.totalRPE <= 8 ? "default" : "destructive"}
                  >
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={progressData}>
              ...
              <YAxis domain={[60, 85]} />
              <Tooltip />
              <Bar dataKey="serveAccuracy" fill="var(--color-secondary)" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
      </Card>
