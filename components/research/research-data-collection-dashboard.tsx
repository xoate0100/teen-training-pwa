...
export function ResearchDataCollectionDashboard() {
  ...
  const [selectedSurveyType, setSelectedSurveyType] = useState<'onboarding' | 'weekly' | 'monthly' | 'quarterly' | 'annual' | 'exit'>('weekly');
  const [selectedTrackingPeriod, setSelectedTrackingPeriod] = useState<'3_months' | '6_months' | '1_year' | '2_years' | '5_years'>('3_months');
  const [autoCollection, setAutoCollection] = useState(true);
  const [activeTab, setActiveTab] = useState<'anonymous' | 'studies' | 'satisfaction' | 'outcomes'>('anonymous');
  ...
  }, [autoCollection, selectedUserId, selectedSurveyType, selectedTrackingPeriod, collectAnonymousPerformanceData, collectUserSatisfactionMetrics, trackLongTermOutcomes]);
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              >
                {autoCollection ? <Pause className="h-4 w-4 mr-2" /> : <Play className="h-4 w-4 mr-2" />}
                {autoCollection ? 'Stop Auto-Collection' : 'Start Auto-Collection'}
              </Button>
              <Button
                variant="outline"
                size="sm"
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="text-sm font-medium mb-2 block">Tracking Period</label>
              <select
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Anonymous Performance Data ({anonymousPerformanceData.length} records)
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              {anonymousPerformanceData.map((data, index) => (
                <div key={index} className="p-4 border rounded-lg">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <Shield className="h-5 w-5" />
                      <h4 className="font-medium">Anonymous User: {data.anonymousUserId}</h4>
                    </div>
                    <div className="flex gap-2">
                      <Badge variant="outline">{data.sessionType}</Badge>
                      <Badge variant="outline">Quality: {(data.dataQuality * 100).toFixed(0)}%</Badge>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                    <div>
                      <span className="text-sm text-muted-foreground">Duration:</span>
                      <span className="ml-2 font-medium">{data.duration} min</span>
                    </div>
                    <div>
                      <span className="text-sm text-muted-foreground">Intensity:</span>
                      <span className="ml-2 font-medium">{data.intensity.toFixed(1)}/10</span>
                    </div>
                    <div>
                      <span className="text-sm text-muted-foreground">Exercises:</span>
                      <span className="ml-2 font-medium">{data.exercises.length}</span>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                    <div>
                      ...
                      <div className="text-sm">
                        {data.demographics.ageGroup} | {data.demographics.experienceLevel} | {data.demographics.trainingFrequency} | {data.demographics.sport}
                      </div>
                    </div>
                    <div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Training Effectiveness Study: {trainingEffectivenessStudy.studyName}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-bold">{trainingEffectivenessStudy.participants}</div>
                  <div className="text-sm text-muted-foreground">Participants</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold">{trainingEffectivenessStudy.duration}</div>
                  <div className="text-sm text-muted-foreground">Weeks</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold">{trainingEffectivenessStudy.studyType}</div>
                  <div className="text-sm text-muted-foreground">Study Type</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold">{trainingEffectivenessStudy.publicationStatus}</div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="text-sm">{trainingEffectivenessStudy.methodology.design}</div>
                  </div>
                  <div>
                    <span className="text-sm text-muted-foreground">Variables:</span>
                    <div className="text-sm">{trainingEffectivenessStudy.methodology.variables.join(', ')}</div>
                  </div>
                  <div>
                    <span className="text-sm text-muted-foreground">Controls:</span>
                    <div className="text-sm">{trainingEffectivenessStudy.methodology.controls.join(', ')}</div>
                  </div>
                  <div>
                    <span className="text-sm text-muted-foreground">Measurements:</span>
                    <div className="text-sm">{trainingEffectivenessStudy.methodology.measurements.join(', ')}</div>
                  </div>
                </div>
              </div>
                    ...
                    {trainingEffectivenessStudy.results.map((result, index) => (
                      <div key={index} className="p-3 border rounded-lg">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <Badge className={result.improvement > 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm text-muted-foreground">
                          ...
                          <div>Significance: {result.significance.toFixed(3)}</div>
              ...
              {trainingEffectivenessStudy.conclusions.length > 0 && (
                <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    {trainingEffectivenessStudy.conclusions.map((conclusion, index) => (
                      <div key={index} className="p-3 border rounded-lg">
                        <div className="font-medium mb-1">{conclusion.finding}</div>
                        <div className="text-sm text-muted-foreground mb-1">{conclusion.evidence}</div>
                        <div className="text-sm text-muted-foreground mb-1">{conclusion.implications}</div>
                        <div className="text-sm text-muted-foreground">{conclusion.limitations}</div>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-bold">{userSatisfactionMetrics.overallSatisfaction.toFixed(1)}/10</div>
                  <div className="text-sm text-muted-foreground">Overall Satisfaction</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold">{userSatisfactionMetrics.netPromoterScore}</div>
                  <div className="text-sm text-muted-foreground">Net Promoter Score</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold">{(userSatisfactionMetrics.completionRate * 100).toFixed(0)}%</div>
                  <div className="text-sm text-muted-foreground">Completion Rate</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold">{userSatisfactionMetrics.responseTime.toFixed(0)} min</div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                  {Object.entries(userSatisfactionMetrics.categoryScores).map(([category, score]) => (
                    <div key={category} className="text-center">
                      <div className="text-lg font-bold">{score.toFixed(1)}/10</div>
                  ...
                  {userSatisfactionMetrics.responses.map((response, index) => (
                    <div key={index} className="p-3 border rounded-lg">
                      ...
                      {response.textResponse && (
                        <div className="text-sm text-muted-foreground">{response.textResponse}</div>
                      )}
                    </div>
                  ))}
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <h5 className="font-medium text-green-600 mb-2">Positive</h5>
                    <div className="space-y-1">
                      {userSatisfactionMetrics.qualitativeFeedback.positive.map((feedback, index) => (
                        <div key={index} className="text-sm text-muted-foreground">• {feedback}</div>
                      ))}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="space-y-1">
                      {userSatisfactionMetrics.qualitativeFeedback.negative.map((feedback, index) => (
                        <div key={index} className="text-sm text-muted-foreground">• {feedback}</div>
                      ))}
                    </div>
                  </div>
                  <div>
                    <h5 className="font-medium text-blue-600 mb-2">Suggestions</h5>
                    <div className="space-y-1">
                      {userSatisfactionMetrics.qualitativeFeedback.suggestions.map((feedback, index) => (
                        <div key={index} className="text-sm text-muted-foreground">• {feedback}</div>
                      ))}
                    </div>
                  </div>
          ...
          <CardContent>
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.entries(longTermOutcomes.healthMetrics).map(([metric, data]) => (
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.entries(longTermOutcomes.psychologicalMetrics).map(([metric, data]) => (
                    <div key={metric} className="p-3 border rounded-lg">
                      <div className="flex justify-between items-center mb-2">
                        <span className="font-medium capitalize">{metric}</span>
                        <div className="flex items-center gap-1">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                  {Object.entries(longTermOutcomes.behavioralChanges).map(([behavior, value]) => (
                    <div key={behavior} className="text-center">
                      <div className="text-lg font-bold">{value.toFixed(1)}/10</div>
                      <div className="text-sm text-muted-foreground capitalize">{behavior.replace(/([A-Z])/g, ' $1').trim()}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="space-y-1">
                      {longTermOutcomes.outcomes.primary.map((outcome, index) => (
                        <div key={index} className="flex justify-between items-center">
                          <span className="text-sm">{outcome.outcome}</span>
                          <div className="flex gap-2">
                            <Badge className={outcome.achieved ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}>
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {longTermOutcomes.outcomes.secondary.map((outcome, index) => (
                        <div key={index} className="flex justify-between items-center">
                          <span className="text-sm">{outcome.outcome}</span>
                          <div className="flex gap-2">
                            <Badge className={outcome.achieved ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}>
