...
import { AlertTriangle, Heart, Activity, Phone, Shield, Thermometer, Clock, CheckCircle } from "lucide-react"
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setMetrics((prev) => ({
        ...
        hydrationReminder: prev.sessionDuration > 20 && prev.sessionDuration % 15 === 0,
      }))
    ...
    <div className="space-y-4">
      {showAlert && (
        <Alert className="border-red-200 bg-red-50">
          ...
          <AlertDescription className="text-red-800">
            High fatigue detected. Consider taking a break or reducing intensity.
          </AlertDescription>
          <Button size="sm" className="mt-2" onClick={() => setShowAlert(false)}>
            Take Break
          </Button>
        </Alert>
      )}
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <Badge className={`${getHeartRateColor(metrics.heartRateZone)} text-white capitalize`}>
              {metrics.heartRateZone}
            </Badge>
            <p className="text-xs text-muted-foreground mt-1">Target: Moderate-Vigorous</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-lg font-semibold">{metrics.sessionDuration} min</p>
            <p className="text-xs text-muted-foreground">Last break: {metrics.lastBreak} min ago</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <Badge variant={metrics.injuryRisk === "low" ? "default" : "destructive"}>{metrics.injuryRisk}</Badge>
            <p className="text-xs text-muted-foreground mt-1">Based on fatigue & form</p>
          </CardContent>
        </Card>
      </div>
...
export function EmergencyContacts() {
  const contacts = [
    ...
    { name: "Parent/Guardian", phone: "(555) 987-6543", role: "Emergency Contact" },
    { name: "Athletic Trainer", phone: "(555) 456-7890", role: "Medical Support" },
  ]
