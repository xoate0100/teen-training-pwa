...
import { Pause, CheckCircle, Timer, Zap, MessageSquare, Star, ArrowRight, Trophy } from "lucide-react"
...
export function SessionCard({
  ...
  const isSessionComplete = exercises.every((ex) => isExerciseComplete(ex.id, ex.sets))

  return (
    <div className="space-y-4">
      {/* Session Header */}
      <Card>
        <CardHeader className={`${sessionColors[sessionType]} text-white rounded-t-lg`}>
      ...
      {isSessionComplete && (
        <Card className="border-green-500 bg-green-50">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-green-700 mb-4">Great work! You've completed all exercises.</p>
            <Button className="bg-green-600 hover:bg-green-700">View Session Summary</Button>
          </CardContent>
        </Card>
      )}
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">
        {exercises.map((ex, index) => (
          ...
          >
            {isExerciseComplete(ex.id, ex.sets) && <CheckCircle className="h-3 w-3 mr-1" />}
            {index + 1}
          </Button>
        ))}
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4 p-4 bg-muted rounded-lg">
              ...
              <div className="text-center">
                <p className="text-2xl font-bold text-primary">{exercise.load || "BW"}</p>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-5 gap-2">
                {Array.from({ length: exercise.sets }, (_, index) => (
                  <Button
                    key={index}
                    variant={(completedSets[exercise.id] || 0) > index ? "default" : "outline"}
                    size="sm"
                    onClick={() => handleSetComplete(exercise.id)}
                    disabled={(completedSets[exercise.id] || 0) > index}
                    className="aspect-square"
                  >
                    {(completedSets[exercise.id] || 0) > index ? <CheckCircle className="h-4 w-4" /> : index + 1}
                  </Button>
                ))}
              </div>
            </div>
              ...
              <Card className="border-green-500 bg-green-50">
                <CardContent className="p-4">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <span className="font-medium text-green-800">Exercise Complete!</span>
                  </div>
                  <p className="text-sm text-green-700 mb-3">Please rate your effort level before moving on.</p>
                  {/* RPE Rating for completed exercise */}
                  <div className="space-y-2">
                    <div className="flex items-center gap-2">
                      <Zap className="h-4 w-4 text-primary" />
                      <span className="font-medium">Final RPE: {rpe[0]}/10</span>
                    </div>
                    <Slider value={rpe} onValueChange={setRpe} max={10} min={1} step={1} className="w-full" />
                  </div>
                </CardContent>
              </Card>
            )}
            ...
            {!isExerciseComplete(exercise.id, exercise.sets) && (
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <Zap className="h-4 w-4 text-primary" />
                  <h4 className="font-medium">Effort Level (RPE): {rpe[0]}/10</h4>
                </div>
                <Slider value={rpe} onValueChange={setRpe} max={10} min={1} step={1} className="w-full" />
            ...
            <div className="space-y-3 pt-4">
              {autoAdvanceEnabled && !isExerciseComplete(exercise.id, exercise.sets) && (
                <div className="flex items-center gap-2 text-sm text-muted-foreground bg-muted/50 p-2 rounded">
                  <ArrowRight className="h-4 w-4" />
                  <span>Auto-advance enabled - will move to next exercise when complete</span>
                </div>
              )}
      ...
      {(currentExercise + 1) % 3 === 0 && currentExercise < exercises.length - 1 && (
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Take 2 minutes to hydrate and do some light stretching</p>
              </div>
            </div>
          </CardContent>
        </Card>
