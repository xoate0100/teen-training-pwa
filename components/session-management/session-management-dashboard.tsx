...
export function SessionManagementDashboard({
  ...
  const selectedSchedule =
    selectedWeek === 'current' ? currentWeekSchedule : nextWeekSchedule;
      ...
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-muted-foreground'>
            AI-powered scheduling, conflict resolution, and optimal timing
            recommendations
          </p>
        </div>
        <div className='flex gap-2'>
          ...
          <Button
            onClick={() =>
              generateWeeklySchedule(new Date().toISOString().split('T')[0])
            }
      ...
      {selectedSchedule && (
        <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
          ...
          <Card>
            ...
            <CardContent>
              <div className='text-2xl font-bold'>
                {selectedSchedule.recoveryDays.length}
              </div>
              <p className='text-xs text-muted-foreground'>Planned rest days</p>
            </CardContent>
          </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className='flex items-center gap-2'>
              ...
              {selectedWeek === 'current' ? 'Current Week' : 'Next Week'}{' '}
              Schedule
              <Badge
                variant={selectedSchedule.optimal ? 'default' : 'destructive'}
              >
                ...
                >
                  <div>
                    ...
                    {conflict.newDate && (
                      <div className='text-sm text-muted-foreground'>
                        New date:{' '}
                        {new Date(conflict.newDate).toLocaleDateString()}
                      </div>
                    )}
                    {conflict.newTime && (
                      <div className='text-sm text-muted-foreground'>
                  ...
                  <Badge
                    variant={
                      conflict.impact === 'high' ? 'destructive' : 'secondary'
                    }
                  >
              ...
              {missedSessions.map(recovery => (
                ...
                >
                  <div>
                    ...
                    <div className='text-sm text-muted-foreground'>
                      Original:{' '}
                      {new Date(recovery.originalDate).toLocaleDateString()}
                    </div>
                    <div className='text-sm text-muted-foreground'>
                      Recovery:{' '}
                      {new Date(recovery.recoveryDate).toLocaleDateString()} at{' '}
                  ...
                  <div className='flex items-center gap-2'>
                    <Badge variant='outline'>
                      {recovery.adjustedIntensity}
                    </Badge>
                    <Badge
                      variant={recovery.optimal ? 'default' : 'destructive'}
                    >
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
              ...
              <div>
                ...
                <div className='space-y-2'>
                  {timingRecommendations.bestTimes
                    ?.slice(0, 3)
              ...
              <div>
                ...
                <div className='space-y-2'>
                  {timingRecommendations.weeklyPattern
                    ?.slice(0, 3)
