...
import { WearableDevice, HeartRateData, SleepData, ActivityData, RecoveryMetrics } from '@/lib/services/wearable-device-integration';
...
export function WearableDeviceDashboard() {
  ...
  const [latestRecovery, setLatestRecovery] = useState<RecoveryMetrics | null>(null);
  ...
  }, [getHeartRateData, getSleepData, getActivityData, getRecoveryMetrics, getLatestRecoveryMetrics]);
  ...
  const getDeviceIcon = (type: string) => {
    switch (type) {
      ...
      case 'chest_strap': return <Heart className="h-5 w-5" />;
      case 'earbuds': return <Headphones className="h-5 w-5" />;
  ...
  const getDeviceTypeLabel = (type: string) => {
    switch (type) {
      ...
      case 'chest_strap': return 'Chest Strap';
    ...
    <div className="space-y-6">
          ...
          <div className="space-y-4">
              ...
              <div className="p-4 border rounded-lg bg-gray-50">
                ...
                <div className="mt-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {['heart_rate', 'sleep', 'activity', 'recovery', 'gps', 'notifications'].map(capability => (
                ...
                <div className="flex gap-2 mt-4">
                  <Button onClick={handleAddDevice} disabled={!newDevice.name || !newDevice.brand || !newDevice.model}>
                    Add Device
                  </Button>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {device.brand} {device.model} • {getDeviceTypeLabel(device.type)}
                      </p>
            ...
            {devices.length === 0 && (
              <div className="text-center text-muted-foreground py-8">
                No devices connected. Add a device to start tracking your health metrics.
              </div>
            )}
          </div>
        </CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {heartRateData.length > 0 ? Math.round(heartRateData[heartRateData.length - 1].bpm) : '--'} BPM
            </div>
            <p className="text-xs text-muted-foreground">
              {heartRateData.length > 0 ? heartRateData[heartRateData.length - 1].zone.replace('_', ' ') : 'No data'}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {sleepData.length > 0 ? formatDuration(sleepData[sleepData.length - 1].totalSleep) : '--'}
            </div>
            <p className="text-xs text-muted-foreground">
              {sleepData.length > 0 ? `${Math.round(sleepData[sleepData.length - 1].sleepScore)} score` : 'No data'}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {activityData.length > 0 ? activityData[activityData.length - 1].steps.toLocaleString() : '--'}
            </div>
            <p className="text-xs text-muted-foreground">
              {activityData.length > 0 ? `${formatDistance(activityData[activityData.length - 1].distance)}` : 'No data'}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold">{latestRecovery.restingHeartRate} BPM</div>
                <p className="text-sm text-muted-foreground">Resting HR</p>
              </div>
              <div className="text-center">
                <div className="text-2xl font-bold">{latestRecovery.stressLevel}%</div>
            ...
            <div>
              <label className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => updateConfig({ enableHeartRate: e.target.checked })}
                />
            ...
            <div>
              <label className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => updateConfig({ enableSleepTracking: e.target.checked })}
                />
            ...
            <div>
              <label className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => updateConfig({ enableActivityTracking: e.target.checked })}
                />
            ...
            <div>
              <label className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => updateConfig({ enableRecoveryMetrics: e.target.checked })}
                />
