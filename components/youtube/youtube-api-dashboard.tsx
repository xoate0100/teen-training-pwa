...
import { VideoRecommendationContext, VideoRecommendation } from '@/lib/services/youtube-api-integration';
...
export function YouTubeAPIDashboard() {
  ...
  const [recommendationContext, setRecommendationContext] = useState<VideoRecommendationContext>({
  ...
  const [cacheStats, setCacheStats] = useState({ videoCount: 0, searchCount: 0, memoryUsage: 0 });
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(value: any) => setRecommendationContext(prev => ({ ...prev, exerciseType: value }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(value: any) => setRecommendationContext(prev => ({ ...prev, skillLevel: value }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(value: any) => setRecommendationContext(prev => ({ ...prev, sessionDuration: value }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setRecommendationContext(prev => ({
                    ...prev,
                    preferences: { ...prev.preferences, maxDuration: parseInt(e.target.value) }
                  }))}
                  min="1"
                  max="120"
                />
              ...
              <div>
                <label className="text-sm font-medium mb-2 block">Min View Count</label>
                <Input
                  type="number"
                  value={recommendationContext.preferences.minViewCount}
                  onChange={(e) => setRecommendationContext(prev => ({
                    ...prev,
                    preferences: { ...prev.preferences, minViewCount: parseInt(e.target.value) }
                  }))}
                  min="0"
                />
              </div>
              ...
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    checked={recommendationContext.preferences.includeFormDemonstrations}
                    onChange={(e) => setRecommendationContext(prev => ({
                      ...prev,
                      preferences: { ...prev.preferences, includeFormDemonstrations: e.target.checked }
                    }))}
                  />
                ...
                <label className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    checked={recommendationContext.preferences.includeProgressions}
                    onChange={(e) => setRecommendationContext(prev => ({
                      ...prev,
                      preferences: { ...prev.preferences, includeProgressions: e.target.checked }
                    }))}
                  />
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold">{Math.round(cacheStats.memoryUsage / 1024)}KB</div>
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {videos.map(video => (
                <div
                  ...
                  onClick={() => {
                    const recommendation = recommendations.find(rec => rec.video.id === video.id);
                    if (recommendation) {
            ...
            {selectedVideo ? (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  />
                  <h3 className="text-lg font-semibold">{selectedVideo.video.title}</h3>
                  <p className="text-muted-foreground mt-1">{selectedVideo.video.channelTitle}</p>
                  
                  {/* Relevance Score */}
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-sm">Relevance:</span>
                    <Badge className={getRelevanceBadge(selectedVideo.relevanceScore)}>
                ...
                {selectedVideo.reasons.length > 0 && (
                  <div>
                    <h4 className="font-medium mb-2">Why this video is recommended:</h4>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground space-y-1">
                      <div>Duration: {formatDuration(selectedVideo.video.duration)}</div>
                      <div>Views: {formatViewCount(selectedVideo.video.viewCount)}</div>
                      <div>Likes: {formatViewCount(selectedVideo.video.likeCount)}</div>
                      <div>Published: {new Date(selectedVideo.video.publishedAt).toLocaleDateString()}</div>
                    </div>
                  </div>
                  <div>
                    <h4 className="font-medium mb-2">Content Analysis</h4>
                    <div className="text-sm text-muted-foreground space-y-1">
                      <div>Form Demo: {selectedVideo.metadata.isFormDemonstration ? 'Yes' : 'No'}</div>
                      <div>Progression: {selectedVideo.metadata.isProgression ? 'Yes' : 'No'}</div>
                      <div>Difficulty: {selectedVideo.metadata.difficultyLevel}</div>
                      <div>Equipment: {selectedVideo.metadata.equipmentRequired.join(', ') || 'None'}</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex gap-2 pt-4 border-t">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => window.open(`https://www.youtube.com/watch?v=${selectedVideo.video.id}`, '_blank')}
                  >
              ...
              {recommendations.slice(0, 12).map((recommendation, index) => (
                ...
                >
                  <div className="flex gap-3">
                    ...
                    />
                    <div className="flex-1 min-w-0">
                      <h4 className="font-medium text-sm line-clamp-2">{recommendation.video.title}</h4>
                      <p className="text-xs text-muted-foreground mt-1">{recommendation.video.channelTitle}</p>
                      <div className="flex items-center gap-2 mt-2">
                        <Badge className={getRelevanceBadge(recommendation.relevanceScore)}>
