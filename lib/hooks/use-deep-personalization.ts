...
export function useDeepPersonalization(): UseDeepPersonalizationReturn {
  ...
  const [challengeLevelProfile, setChallengeLevelProfile] = useState<ChallengeLevelProfile | null>(null);
  const [supportSystemProfile, setSupportSystemProfile] = useState<SupportSystemProfile | null>(null);
  ...
  useEffect(() => {
    const updateData = () => {
      setLearningStyleProfile(deepPersonalization.getLearningStyleProfile('current-user'));
      setMotivationProfile(deepPersonalization.getMotivationProfile('current-user'));
      setChallengeLevelProfile(deepPersonalization.getChallengeLevelProfile('current-user'));
      setSupportSystemProfile(deepPersonalization.getSupportSystemProfile('current-user'));
      setPersonalizationInsights(deepPersonalization.getPersonalizationInsights('current-user'));
    };
  ...
  const analyzeLearningStyle = useCallback(async (userId: string) => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to analyze learning style';
  ...
  const analyzeMotivationPatterns = useCallback(async (userId: string) => {
    try {
      ...
      const profile = await deepPersonalization.analyzeMotivationPatterns(userId);
      setMotivationProfile(profile);
      
      return profile;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to analyze motivation patterns';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setIsLoading(false);
  ...
  const analyzeChallengeLevel = useCallback(async (userId: string) => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to analyze challenge level';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setIsLoading(false);
  ...
  const analyzeSupportSystem = useCallback(async (userId: string) => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to analyze support system';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setIsLoading(false);
  ...
  const generatePersonalizationInsights = useCallback(async (userId: string) => {
    try {
      setIsLoading(true);
      setError(null);
      
      const insights = await deepPersonalization.generatePersonalizationInsights(userId);
      setPersonalizationInsights(insights);
      
      return insights;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Failed to generate personalization insights';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setIsLoading(false);
