...
export function useExerciseDBIntegration(): UseExerciseDBIntegrationReturn {
    ...
    const initializeData = async () => {
      try {
        ...
        const initialExercises =
          await exerciseDBIntegration.searchExercises('');
        setExercises(initialExercises);
      } catch (err) {
        setError(
          err instanceof Error
            ? err.message
            : 'Failed to initialize exercise data'
        );
      } finally {
        setIsLoading(false);
      }
  ...
  const searchExercises = useCallback(async (query: string, filters?: any) => {
    try {
      ...
      const results = await exerciseDBIntegration.searchExercises(
        query,
        filters
      );
  ...
  const getSmartRecommendations = useCallback(
    async (context: RecommendationContext, count: number = 5) => {
      try {
        setIsLoading(true);
        setError(null);

        const results = await exerciseDBIntegration.getSmartRecommendations(
          context,
          count
        );
  ...
  const getAlternativeExercises = useCallback(
    async (exerciseId: string, context: RecommendationContext) => {
      try {
        setIsLoading(true);
        setError(null);

        const results = await exerciseDBIntegration.getAlternativeExercises(
          exerciseId,
          context
        );
        ...
        setError(
          err instanceof Error
            ? err.message
            : 'Failed to get alternative exercises'
        );
        return [];
      } finally {
        setIsLoading(false);
  ...
  const getProgressionExercises = useCallback(
    async (exerciseId: string, context: RecommendationContext) => {
      try {
        setIsLoading(true);
        setError(null);

        const results = await exerciseDBIntegration.getProgressionExercises(
          exerciseId,
          context
        );
        ...
        setError(
          err instanceof Error
            ? err.message
            : 'Failed to get progression exercises'
        );
        return [];
      } finally {
        setIsLoading(false);
  ...
  const analyzeExerciseCompatibility = useCallback(
    async (exerciseId: string, context: RecommendationContext) => {
      try {
        setError(null);
        return await exerciseDBIntegration.analyzeExerciseCompatibility(
          exerciseId,
          context
        );
      } catch (err) {
        setError(
          err instanceof Error ? err.message : 'Failed to analyze compatibility'
        );
        return {
          compatibility: 0,
