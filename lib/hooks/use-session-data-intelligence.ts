...
export function useSessionDataIntelligence(): UseSessionDataIntelligenceReturn {
  // State
  const [realTimeTracking, setRealTimeTracking] =
    useState<RealTimeTracking | null>(null);
  const [performanceMetrics, setPerformanceMetrics] =
    useState<PerformanceMetrics | null>(null);
  const [formQuality, setFormQuality] = useState<FormQualityAssessment | null>(
    null
  );
  const [rpeAnalysis, setRpeAnalysis] = useState<RPEAnalysis | null>(null);
  const [safetyMonitoring, setSafetyMonitoring] =
    useState<SafetyMonitoring | null>(null);
  ...
  const assessFormQuality = useCallback(
    async (
      exerciseId: string,
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : 'Failed to assess form quality'
        );
      } finally {
  ...
  const monitorSafety = useCallback(
    async (
      sessionId: string,
      ...
      try {
        ...
        setError(
          err instanceof Error ? err.message : 'Failed to monitor safety'
        );
      } finally {
  ...
  const getRealTimeTracking = useCallback(async (sessionId: string) => {
    ...
    try {
      const tracking =
        await sessionDataIntelligence.getRealTimeTracking(sessionId);
      setRealTimeTracking(tracking);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : 'Failed to get real-time tracking'
      );
    } finally {
      setIsTracking(false);
    }
