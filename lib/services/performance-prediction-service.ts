...
export class PerformancePredictionService {
  ...
  static predictStrengthProgression(
    ...
    const strengthSessions = sessions.filter(s => s.type === 'strength' && s.completed);
    ...
    const progressionRate = this.calculateProgressionRate(strengthSessions, factors);
    
    // Calculate predicted level and time to goal
    const predictedLevel = currentLevel * (1 + progressionRate);
    const timeToGoal = goalStrength > currentLevel 
      ? Math.ceil((goalStrength - currentLevel) / (currentLevel * progressionRate))
      : 0;
    
    // Calculate confidence based on data quality and consistency
    const confidence = this.calculatePredictionConfidence(strengthSessions, factors);
  ...
  static predictFatigueLevels(
    ...
    const predictedFatigue = this.calculatePredictedFatigue(sessions, checkIns, currentFatigue);
    
    // Calculate time to recovery
    const timeToRecovery = this.calculateTimeToRecovery(predictedFatigue, checkIns);
    
    // Assess risk level
    const riskLevel = this.assessFatigueRiskLevel(predictedFatigue);
    
    // Generate recommendations
    const recommendations = this.generateFatigueRecommendations(predictedFatigue, riskLevel);
  ...
  static assessInjuryRisk(
    ...
    const riskScore = this.calculateRiskScore(trainingLoad, recoveryQuality, formConsistency, previousInjuries);
    
    // Determine risk level
    const overallRisk = riskScore < 30 ? 'low' : riskScore < 70 ? 'medium' : 'high';
    
    // Generate recommendations and prevention strategies
    const recommendations = this.generateInjuryRiskRecommendations(overallRisk, {
      trainingLoad,
      recoveryQuality,
      formConsistency,
      previousInjuries,
    ...
    const preventionStrategies = this.generatePreventionStrategies(overallRisk, {
      trainingLoad,
      recoveryQuality,
      formConsistency,
      previousInjuries,
  ...
  static calculateOptimalLoad(
    ...
    const recommendedRPE = Math.min(10, Math.max(1, currentRPE * phaseMultipliers.rpe));
    const recommendedDuration = Math.round(currentDuration * phaseMultipliers.duration);
    const recommendedFrequency = Math.round(currentFrequency * phaseMultipliers.frequency);
    ...
    const deloadFrequency = this.calculateDeloadFrequency(sessions, currentPhase);
  ...
  static generatePerformanceForecast(
    ...
    const strength = this.predictStrengthProgression(sessions, checkIns, goalStrength);
    const fatigue = this.predictFatigueLevels(sessions, checkIns);
    const injuryRisk = this.assessInjuryRisk(sessions, checkIns);
    const optimalLoad = this.calculateOptimalLoad(sessions, checkIns, currentPhase);
  ...
  // Helper methods
  private static calculateCurrentStrengthLevel(sessions: SessionData[]): number {
    ...
    const avgRPE = sessions.reduce((sum, s) => sum + s.totalRPE, 0) / sessions.length;
  ...
  private static calculateProgressionRate(
    ...
    const adjustedRate = baseRate * frequencyMultiplier * intensityMultiplier * consistencyMultiplier * recoveryMultiplier;
  ...
  private static calculateCurrentFatigue(sessions: SessionData[], checkIns: CheckInData[]): number {
    ...
    const avgSoreness = recentCheckIns.reduce((sum, c) => sum + c.soreness, 0) / recentCheckIns.length;
  ...
  private static calculatePredictedFatigue(
    ...
    const predictedFatigue = currentFatigue + (trainingLoad * (1 - recoveryQuality));
    
    return Math.min(10, Math.max(0, predictedFatigue));
  }

  private static calculateTimeToRecovery(predictedFatigue: number, checkIns: CheckInData[]): number {
  ...
  private static assessFatigueRiskLevel(predictedFatigue: number): 'low' | 'medium' | 'high' {
  ...
  private static generateFatigueRecommendations(
    ...
    if (riskLevel === 'high') {
      ...
      recommendations.push('Focus on sleep quality and nutrition for better recovery');
      recommendations.push('Consider light stretching or mobility work instead of intense training');
    } else if (riskLevel === 'medium') {
      recommendations.push('Monitor your energy levels closely during training');
  ...
  private static calculateFormConsistency(sessions: SessionData[]): number {
    ...
    const variance = rpeValues.reduce((sum, r) => sum + Math.pow(r - mean, 2), 0) / rpeValues.length;
  ...
  private static generateInjuryRiskRecommendations(
    ...
    if (riskLevel === 'high') {
      ...
      recommendations.push('Consider consulting a sports medicine professional');
    } else if (riskLevel === 'medium') {
  ...
  private static calculateDeloadFrequency(sessions: SessionData[], phase: string): number {
    ...
    return phaseDeloadFrequencies[phase as keyof typeof phaseDeloadFrequencies] || 4;
  }
  ...
  private static generateNextSessionRecommendation(
    ...
    if (fatigue.riskLevel === 'low' && injuryRisk.overallRisk === 'low') {
      intensity = 'high';
    } else if (fatigue.riskLevel === 'high' || injuryRisk.overallRisk === 'high') {
      intensity = 'low';
    }
    
    // Determine duration
    const duration = Math.round(optimalLoad.recommendedDuration * (intensity === 'high' ? 1.1 : intensity === 'low' ? 0.8 : 1.0));
