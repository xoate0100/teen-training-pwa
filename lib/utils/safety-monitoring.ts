...
import { DailyCheckIn, SetLog, Session, SafetyAlert } from '@/lib/types/database'
...
export function analyzeSafetyMetrics(
  ...
  if (recentCheckIns.length > 0) {
    const avgEnergy = recentCheckIns.reduce((sum, check) => sum + check.energy_level, 0) / recentCheckIns.length
    const avgSoreness = recentCheckIns.reduce((sum, check) => sum + check.muscle_soreness, 0) / recentCheckIns.length
    const avgSleep = recentCheckIns.reduce((sum, check) => sum + check.sleep_hours, 0) / recentCheckIns.length
  ...
  if (recentSetLogs.length > 0) {
    const avgRPE = recentSetLogs.reduce((sum, log) => sum + log.rpe, 0) / recentSetLogs.length
    const highRPEPercentage = recentSetLogs.filter(log => log.rpe > 8).length / recentSetLogs.length
  ...
  if (recentSessions.length >= 2) {
    ...
    if (recentSessionsWithWeights.length >= 2) {
      ...
      if (load_progression > DEFAULT_SAFETY_THRESHOLDS.max_weight_increase_per_week) {
  ...
  const overtrainingScore = checkOvertrainingPatterns(recentSessions, recentCheckIns)
  if (overtrainingScore > 0.7) {
    recommendations.push("Signs of overtraining detected - take additional rest")
 ...
 * Check for overtraining patterns
 */
function checkOvertrainingPatterns(sessions: Session[], checkIns: DailyCheckIn[]): number {
  ...
  if (maxConsecutive > DEFAULT_SAFETY_THRESHOLDS.max_consecutive_high_intensity) {
  ...
  if (checkIns.length >= 3) {
    ...
    const decliningEnergy = recentCheckIns.every((check, index) => 
      index === 0 || check.energy_level < recentCheckIns[index - 1].energy_level
    )
    
    const increasingSoreness = recentCheckIns.every((check, index) => 
      index === 0 || check.muscle_soreness >= recentCheckIns[index - 1].muscle_soreness
    )
...
export function generateSafetyAlerts(
  ...
  if (metrics.fatigue_level > 8) {
    ...
    alerts.push({
      ...
      alert_type: 'fatigue',
      severity: 'high',
      message: 'Elevated fatigue levels. Monitor closely and reduce intensity if needed.',
      is_resolved: false
    })
  }
  ...
  if (metrics.form_quality < 3) {
    alerts.push({
      ...
      alert_type: 'form',
      severity: 'critical',
      message: 'Poor form quality detected. Stop current exercise and focus on technique.',
      is_resolved: false
    })
    ...
    alerts.push({
      ...
      alert_type: 'form',
      severity: 'high',
      message: 'Form quality declining. Reduce weight and focus on proper technique.',
      is_resolved: false
    })
  }
  ...
  if (metrics.injury_risk === 'critical') {
    alerts.push({
      ...
      severity: 'critical',
      message: 'High injury risk detected. Consider consulting a healthcare professional.',
      is_resolved: false
    })
    ...
    alerts.push({
      ...
      alert_type: 'injury_risk',
      severity: 'high',
      message: 'Elevated injury risk. Take additional precautions and rest if needed.',
      is_resolved: false
    })
  }
