...
async function runAPITests() {
  ...
  if (exercisesResult.success) {
    results.passed++
    console.log(`   Found ${exercisesResult.data?.data?.length || 0} exercises`)
  } else {
    results.failed++
  }
  ...
  // Test 2: Get sessions
  console.log('\n🏋️ Testing Sessions...')
  const sessionsResult = await testAPIEndpoint('GET', `/api/sessions?user_id=${TEST_USER_ID}`)
  results.total++
  ...
  console.log('\n📊 Testing Check-ins...')
  const checkInsResult = await testAPIEndpoint('GET', `/api/check-ins?user_id=${TEST_USER_ID}`)
  results.total++
  ...
  const createCheckInResult = await testAPIEndpoint('POST', '/api/check-ins', newCheckIn)
  results.total++
  ...
  const createSessionResult = await testAPIEndpoint('POST', '/api/sessions', newSession)
  results.total++
